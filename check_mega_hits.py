...
def main():
    ...
    for i in results:
        ...
        sql_str = '''SELECT COUNT(*) FROM draws WHERE ball_1 = ? AND ball_2 = ? AND
            ball_3 = ? AND ball_4 = ? AND ball_5 = ? AND mega_ball = ?'''
        count_val = db.execute_query(db_path, sql_str, balls)
        if count_val[0][0] > 0:
    ...
    for i in results:
        balls = (i[0], i[1], i[2], i[3], i[4], i[5])
        sql_str = '''SELECT COUNT(*) FROM draws_column WHERE ball_1 = ? AND ball_2 = ? AND
            ball_3 = ? AND ball_4 = ? AND ball_5 = ? AND mega_ball = ?'''
        count_val = db.execute_query(db_path, sql_str, balls)
        if count_val[0][0] > 0:
            print('{}: Balls - {}, {}, {}, {}, {}  mega ball {}'.format(
